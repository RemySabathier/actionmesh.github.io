/**
 * Script for the text-to-4D video grid section
 * Handles set switching between different video sets
 * Note: Synchronization is disabled for this section
 */

(function () {
  const grid = document.getElementById('txt-to-4d-grid');
  if (!grid) return; // safety: do nothing if the block isn't on the page

  const setButtons = document.querySelectorAll('.txt-to-4d-set-button');
  const sets = grid.querySelectorAll('.txt-to-4d-video-set');
  ...
  function showSet(index) {
    sets.forEach(set => {
      ...
      if (isTarget) {
        set.classList.add('txt-to-4d-active');
        ...
        set.querySelectorAll('video').forEach(v => {
          ...
          if (v.preload === 'none') {
            ...
            const targetSpeed = window.VIDEO_SPEED_CONFIG?.txt_to_4d || 0.5;
        ...
        set.classList.remove('txt-to-4d-active');
    ...
    setButtons.forEach(btn => {
      if (btn.dataset.set === String(index)) {
        btn.classList.add('txt-to-4d-active');
      } else {
        btn.classList.remove('txt-to-4d-active');
      }
    });
  }
